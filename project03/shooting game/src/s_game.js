        ...
        for (let i = bulletList.length - 1; i >= 0; i--) {
            for (let j = enemyList.length - 1; j >= 0; j--) {
                if (
                    ...
                    bulletList[i].x <= enemyList[j].x + 60
                ) {
                    score++;
...
function render() {
    ...
    ctx.fillStyle = "red";
    ctx.fillText("Fire: " + fireCount, 10, 60);
...
function updateEnemy() {
    if (!isGameOver) {
        for (let i = 0; i < enemyList.length; i++) {
            ...
            //적군이 우주선과 만나면 게임종료(각 값을 -4씩 조정)
            if (
                spaceshipX + 4 <= enemyList[i].x + 60 &&
                spaceshipX + 60 >= enemyList[i].x + 4 &&
                spaceshipY + 4 <= enemyList[i].y + 60 &&
                spaceshipY + 60 >= enemyList[i].y + 4
            ) {
                isGameOver = true;
            }
        }
